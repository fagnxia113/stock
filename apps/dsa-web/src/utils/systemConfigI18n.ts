...
const fieldDescriptionMap: Record<string, string> = {
  ...
  AGENT_ARCH: "选择 Agent 执行架构。multi 为多 Agent 编排默认模式；single 为旧版单 Agent。",
  AGENT_ORCHESTRATOR_MODE: "Multi-Agent 编排深度。quick（技术→决策）、standard（技术→情报→决策）、full（含风控）、debate（含行业、资金、反方审计、辩论和因子评分，默认）、full_debate（再加入基本面与情绪）。",
